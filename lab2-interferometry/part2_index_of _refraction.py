...
theta_err = np.array([0.5/6, 0.5/6, 0.5/6, 0.5/6, 0.5/6, 0.5/6])
dtheta = (theta_i - theta_f) / 180 * np.pi
dtheta_err = (tl.error_add(theta_err, theta_err) / 180) * np.pi
N = np.array([0, 28, 32, 11, 5, 20])
N_err = np.array([1, 1, 1, 1, 1, 1])

initial_guess = 1
bb.plot_fit(quadratic_proportional, dtheta, N, dtheta_err, N_err, init_guess=initial_guess, font_size=12, xlabel="Angle (rad)", ylabel="Number of fringes")

a_fit = 4930.000705260947
a_fit_err = 53.843642206041224
