...
v  = 186.01   # Voltage, ΔV, in V
v_err = 0.01 # Uncertainty in voltage measurement
# Fitted values from fixed-voltage analysis:
a_fit, a_fit_unc = 0.0640095857828037, 0.0016815877487173848

# k and k_err have been computed earlier (in fixed_current.py or extra_field.py)
# For example purposes assume they are already defined:
# k = 1/math.sqrt(2) * (4/5)**(3/2) * mue_0 * n / R
# k_err = k * (R_err/R)
# (Ensure these values are set in your workspace.)

# Calculate e/m using the relation: e/m = ΔV / (a_fit^2 * k^2)
e_ratio_V = v / (a_fit**2 * k**2)

# Compute the combined relative uncertainty:
# Relative uncertainty from a_fit: 2*(a_fit_unc/a_fit)
# from k: 2*(k_err/k)
# from voltage: (v_err/v)
rel_err = np.sqrt((2 * a_fit_unc / a_fit)**2 + (2 * k_err / k)**2 + (v_err / v)**2)

# Calculate the absolute uncertainty:
e_ratio_V_err = e_ratio_V * rel_err
...
weighted_avg = (e_ratio_I/e_ratio_I_err**2 + e_ratio_V/e_ratio_V_err**2)/(1/e_ratio_I_err**2 + 1/e_ratio_V_err**2)
weighted_avg_err = 1/np.sqrt(1/e_ratio_I_err**2 + 1/e_ratio_V_err**2)

print(f"Average e/m ratio: {weighted_avg:.4e} +/- {weighted_avg_err:.4e}")
print(f"Percent error for average: {abs(weighted_avg - accepted)/accepted*100:.3f}%")
...
print("Percent uncertainty for average: ", format(weighted_avg_err/weighted_avg*100,".2f"))
...
plt.errorbar([1,2,3],[e_ratio_I, e_ratio_V,weighted_avg], yerr=[e_ratio_I_err, e_ratio_V_err,weighted_avg_err], fmt='o', label="Calculated values")
